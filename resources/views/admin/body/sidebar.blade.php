...
<aside class="navbar-aside bg-primary" id="offcanvas_aside">
    <div class="aside-top">
        <a href="{{ route('admin.dashboard') }}" class="brand-wrap">
            ...
            @endphp
            @if($logo != null)
                <img src="{{ asset(get_setting('site_footer_logo')->value ?? ' ') }}" alt="{{ env('APP_NAME') }}"  style="height: 95px !important; min-width: 95px !important;">
            @else
                <img src="{{ asset('upload/no_image.jpg') }}" alt="{{ env('APP_NAME') }}" style="height: 95px !important; min-width: 95px !important;">
            @endif
        </a>
        <div>
    ...
    <nav>
        <ul class="menu-aside">
            ...
            <li class="menu-item has-submenu {{ $route == 'all_orders.index'||$route == 'all_orders.pos_sale_index'||$route == 'all_orders.all_vendor_sale_index' ||$route == 'all_orders.all_reseller_sale_index' ? 'active' : '' }}">
                @if (Auth::guard('admin')->user()->role == '1' ||
